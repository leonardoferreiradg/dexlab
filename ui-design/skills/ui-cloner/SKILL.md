...
Use `ui-typeset` and `ui-color-system-creator` to formalize the extracted system as a proper token architecture, `ds-component-spec` to document components, or `ui-wireframer-creator` to produce new screens in the style.
