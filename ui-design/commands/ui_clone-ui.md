...
2. **Extract the design system** — Use `ui_cloner` to systematically analyze: typography (fonts, scale, weights), color palette (approximate oklch values for each role), spacing (base unit, rhythm, card padding), visual treatment (border radius, shadows, borders), and visual personality (theme, tone, density).
...
4. **Produce new screens** — Use `ui_cloner` to generate new UI in the confirmed style. Every value traceable to the reference. No improvisation outside the extracted system.
...
Use `/ui_build-visual-system` to formalize the extracted system as a proper token architecture, `/ds_create-component` to document components from the reference.
