...
1. **Establish naming rules** — Use `ds_naming-convention` to define the naming convention for each token tier. Global: describes raw value. Alias: describes purpose. Component: scoped to component. Agreement on naming before creating any tokens.
...
3. **Create the token architecture** — Use `ds_designtokens` to build three tiers:
...
4. **Document token usage rules** — Use `ds_designtokens` to specify: which tier components reference (alias only), which tokens are theming-safe (can be overridden), which have accessibility constraints (minimum contrast — never override).
...
Use `/ds_create-component` to apply the token system to new components, `/ds_audit-system` to validate the full system.
